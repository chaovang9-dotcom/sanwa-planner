(function(){
  ...
  (function(){
    ...
    function resizeCanvasToWrapper(){
      ...
      const dpr  = Math.min(2, Math.max(1, window.devicePixelRatio || 1)); // clamp DPR on mobile for perf
    ...
    if (window.ResizeObserver){
      ...
      const ro = new ResizeObserver(()=>{
        if (_roBusy) return; _roBusy = true;
        requestAnimationFrame(()=>{ try{ resizeCanvasToWrapper(); } finally { _roBusy=false; } });
      });
